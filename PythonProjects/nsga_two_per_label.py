...
def crowding_distance(acc_list, front):
    ...
    for i in range(len(acc_list)):
        for k in range(1, len(front) - 1):
            if (max(acc_list[i]) - min(acc_list[i])) != 0.0:
                distance[k] = distance[k] + (
                            acc_list[i][sorted_list[i][k + 1]] - acc_list[i][sorted_list[i][k - 1]]) / (
...
def crossover(solutions):
    for i in range(2):
        ...
        r = random.random()
        temp1 = solutions[i][k] * r + solutions[i + 2][k] * (1 - r)
        temp2 = solutions[i][k] * (1 - r) + solutions[i + 2][k] * r
        solutions[i][k] = temp1
        solutions[i + 2][k] = temp2
